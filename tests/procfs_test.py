...
from procfs.procfs import Proc, ProcNet, ProcPid
...
class Test_ProcNet(unittest.TestCase):
    ...
    @patch(
        "builtins.open",
        mock_open(
            read_data="""  sl  local_address rem_address   st tx_queue rx_queue tr tm->when retrnsmt   uid  timeout inode
...
10.0.0.1      0x1         0x2         52:54:00:46:2f:8d     *        enp61s0u1u2\n"""
 ...
 face |bytes    packets errs drop fifo frame compressed multicast|bytes    packets errs drop fifo colls carrier compressed
    ...
    @patch(
        "builtins.open", mock_open(read_data="6    eth0  1     0     01005e00006a\n")
    )
    ...
    @patch(
        "builtins.open",
        mock_open(
            read_data="TcpExt: SyncookiesSent SyncookiesRecv\nTcpExt: 1 2\nIpExt: InNoRoutes InTruncatedPkts\nIpExt: 3 4\n"
        ),
    )
    ...
    @patch(
        "builtins.open",
        mock_open(
            read_data="Ip: Forwarding DefaultTTL\nIp: 1 64\nIcmp: InMsgs InErrors\nIcmp: 4 0\n"
        ),
    )
    def test_snmp(self):
        with Proc() as p:
    ...
    @patch(
        "builtins.open",
        mock_open(
            read_data="Ip6InReceives                   	579556\nIp6InHdrErrors                  	0\n"
        ),
    )
    def test_snmp6(self):
        with Proc() as p:
...
enp61s0u1u2	00000000	00000000	0003	0	0	100	00000000	0	0	0\n"""
    ...
    @patch(
        "builtins.open",
        mock_open(
            read_data="00000000000000000000000000000001 80 00000000000000000000000000000000 00 00000000000000000000000000000000 00000100 00000001 00000000 00000001       lo\n"
        ),
    )
    ...
    @patch(
        "builtins.open",
        mock_open(
            read_data="""Num       RefCount Protocol Flags    Type St Inode Path
...
class Test_ProcPid(unittest.TestCase):
    ...
    @patch(
        "builtins.open",
        mock_open(
            read_data="Limit                     Soft Limit           Hard Limit           Units    \nMax stack size            8388608              unlimited            bytes   \n"
        ),
    )
    ...
    @patch(
        "builtins.open",
        mock_open(
            read_data="560646741000-560646745000 r--p 00000000 fe:01 7078353                    /usr/bin/cat\n"
        ),
    )
    def test_maps(self):
        with ProcPid() as p:
    ...
    def test_smaps(self):
        with patch("procfs.procfs.ProcPid._maps") as mock_maps:
    ...
    @patch(
        "builtins.open",
        mock_open(read_data="tmpfs /dev/shm tmpfs rw,nosuid,nodev,foo=1,bar=baz 0 0\n"),
    )
    ...
    @patch(
        "builtins.open",
        mock_open(
            read_data="24 100 0:6 / /dev rw,nosuid shared:21 - devtmpfs devtmpfs rw,size=16352080k,nr_inodes=4088020,mode=755\n"
        ),
    )
    ...
    @patch(
        "builtins.open",
        mock_open(
            read_data="7f7f335d7000 default file=/lib/x86_64-linux-gnu/ld-2.27.so anon=1 dirty=1 N0=1 kernelpagesize_kB=4\n7ffc19d01000 default\n"
        ),
    )
    def test_numa_maps(self):
        with ProcPid() as p:
            ...
            del p["numa_maps"]
            self.assertEqual(
                p.numa_maps["7f7f335d7000"].file, "/lib/x86_64-linux-gnu/ld-2.27.so"
            )
...
class Test_Proc(unittest.TestCase):
    ...
    @patch(
        "builtins.open",
        mock_open(
            read_data="#subsys_name\thierarchy\tnum_cgroups\tenabled\ncpuset\t8\t4\t1\n"
        ),
    )
    def test_cgroups(self):
        with Proc() as p:
            ...
            self.assertEqual(p.data, {})
            self.assertIs(
                p.cgroups.cpuset.hierarchy, p["cgroups"]["cpuset"]["hierarchy"]
            )
    ...
    @patch(
        "builtins.open",
        mock_open(
            read_data="MemTotal:       32727212 kB\nMemFree:        24443188 kB\n"
        ),
    )
    ...
    @patch(
        "builtins.open",
        mock_open(read_data="nr_free_pages 6097475\nnr_zone_inactive_anon 53530\n"),
    )
    ...
    @patch(
        "builtins.open",
        mock_open(
            read_data="""       key      shmid perms                  size  cpid  lpid nattch   uid   gid  cuid  cgid      atime      dtime      ctime                   rss                  swap\n         0          6  1600                524288  2669  7487      2  1000   100  1000   100 1572344073 1572344073 1572342595                 12288                     0\n"""
        ),
    )
    ...
    @patch(
        "builtins.open",
        mock_open(
            read_data="Linux version 5.3.7-1-default (geeko@buildhost) (gcc version 9.2.1 20190903 [gcc-9-branch revision 275330] (SUSE Linux)) #1 SMP Mon Oct 21 06:03:17 UTC 2019 (3eea5a9)"
        ),
    )
